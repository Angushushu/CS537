...
void printjobs(struct job* jobhead){
	...
	while(currentjob != NULL){
		...
		//int status;
		//waitpid(currentjob->pid, &status, 0);//was NULL or WNOHANG
		printf("--jobs test--\n");
		printf("status: %i\n", *(currentjob->status));
		printf("pid: %i\n", currentjob->pid);
		printf("WIFSIGNALED(status): %i\n",WIFSIGNALED(*(currentjob->status)));
		printf("WIFEXITED(status): %i\n",WIFEXITED(*(currentjob->status)));
		printf("WCOREDUMP(status): %i\n",WCOREDUMP(*(currentjob->status)));
		printf("WIFSTOPPED(status): %i\n",WIFSTOPPED(*(currentjob->status)));
		...
		if(!(WIFEXITED(*(currentjob->status)))){
			write(1, "here's a job\n", 13);
			write(1, &(currentjob->jid), sizeof(currentjob->jid));
        	write(1, " : ", 3);
            	//print command
			...
			if(jobhead!=NULL){
				...
				if(sizeofargs == 2 && thejob!=NULL){
					...
					if(WIFEXITED(*status)||WIFSIGNALED(*status)||WCOREDUMP(*status)||WIFSTOPPED(*status)){//if job terminated
						write(STDOUT_FILENO, "JID ", 4);//repeated
