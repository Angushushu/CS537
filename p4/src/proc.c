...
int addproc(int pri, struct proc* p) {
  for(int i = 0; i < NPROC; i++) {
    if(q.proc[pri][i] == 0) {
      q.proc[pri][i] = p;
      // should be added even there's only 1 proc in the queue
      p->qtail[pri]++;
      p->pri = pri;
      p->runtime[0] = 0;
      p->runtime[1] = 0;
...
found:
  ...
  *(uint*)sp = (uint)trapret;
  ...
  if(p->parent == 0) { //NULL
    addproc(3, p);  // Default priority is 3 (p4)
  } else {
    addproc(p->parent->pri, p);  // set to its parent's priority
  }
  for(int i = 0; i < 4; i++) {
    p->runtime[p->pri] = 0; //
    p->ticks[p->pri] = 0;
...
scheduler(void)
{
  struct proc *p = 0; // initialized
  struct cpu *c = mycpu();
  c->proc = 0;
  // timeslice of each priority
  int timeslice[4] = {20, 16, 12, 8};
  ...
  for(;;){
    ...
    for(int i = 3; i >= 0; i--) {  // pri lvl
      while(q.proc[i][0] != 0) {
        if(q.proc[i][0]->state == RUNNABLE) {// how about RUNNING?
          p = q.proc[i][0];
          goto runproc;
        } else {  // ZOMBIE/UNUSED/EMBRYO
          removefromq(i, q.proc[i][0]->pid);
        }
      }
    }
    release(&ptable.lock);
...
runproc:
    ...
    p->ticks[p->pri]++;
    p->runtime[p->pri]++;
    if(p->runtime[p->pri] == timeslice[p->pri]) {
      p->runtime[p->pri] = 0;
      // add to back of queue if not the last one
      removefromq(p->pri, p->pid);
      addproc(p->pri, p);
    }
    //p->state = RUNNABLE; //
    release(&ptable.lock);
...
sleep(void *chan, struct spinlock *lk)
{
  ...
  // (p4) dont use, if used, need ctrl c for 1, 2, 3
  //removefromq(p->pri, p->pid);
...
wakeup1(void *chan)
{
  ...
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++)
    if(p->state == SLEEPING && p->chan == chan) {
      ...
      //cprintf("wakeup1--\n");
      // (p4) move the waked up proc to back of queue
      //cprintf("%d woke up\n", p->pid);
      //removefromq(p->pri, p->pid);
      addproc(p->pri, p);
    }
