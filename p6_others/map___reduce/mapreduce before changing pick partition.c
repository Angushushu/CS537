...
void insert(struct table* t, char* key, char* val, Partitioner partitioner) { // how to pass method
    ...
    if(list == NULL) {  // if no key yet, add keyNode
        ...
        if(tmp->next == NULL || strcmp(key, tmp->next->key) < 0) {  // if the key doesn't exist yet, add it
            ...
            while(vtmp->next != NULL && strcmp(val, vtmp->next->val) > 0) // while val > vtmp->next->val, keep going
                vtmp = vtmp->next;
            // add valNode to the val_list and don't need to care about repeat
            new_valNode->next = vtmp->next;
            vtmp->next = new_valNode;
        }
    }
    pthread_mutex_unlock(&t->lock[p_num]);
}
struct keyNode* pick_partition(struct table* t) { // pick the next partition to reduce -> change to pick the next partition
...
void* reduce_thread(void * arg) {
    ...
    Reducer reduce = arg;
    struct keyNode* head;
    while((head = pick_partition(hashmap3D)) != NULL) {
        printf(" while in reduce_thread()\n");
        printf(" head->key = %s\n", head->key);
        struct keyNode* tmp = head;
        while(tmp != NULL) {
            char* key = tmp->key;
