...
void addtoQ2(struct Node2* newNode) {
    ...
    while(tmp) {
        if(newNode->key == tmp->key && newNode->key == tmp->value) {
            break;
        }
        if(tmp->next == NULL) {
            tmp->next = newNode;
...
void insert(struct table *t,char* key,char* val, Partitioner partitioner){ // how to pass method
    ...
    int pos = partitioner(t,key);  // get partition number
    ...
    if(list == NULL){ // when first one is empty
        ...
        while(tmp->next != NULL && strcmp(key, tmp->next->key) >= 0) { // while key is not smaller than tmp->next->key
            tmp = tmp->next;
        }
...
void MR_Emit(char *key, char *value) {
    insert(partitions, key, value, partition); // add to partition
}
...
void reduce_thread(Reducer reduce, ReduceArg reduce_arg) {
    char* key = reduce_arg.key;
    Getter get_func = reduce_arg.get_func; // for sure it's get_next(), do I need
    int partition_number = reduce_arg.partition_number;
    while(reduce(key, get_next, partition_number)) {
...
void MR_Run(int argc, char *argv[],
    ...
    for(int i = 0; i < num_reducers; i++)
        Pthread_create(&threads_r[i], NULL, reduce_thread, reduce_arg);
    // wait all threads done
    for(int i = 0; i < num_reducers; i++)
