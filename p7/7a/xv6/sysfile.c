...
sys_unlink(void)
{
  ...
  // seems like no need
  // if(ip->type == T_SYM) {
  //   kfree(ip->addrs);  // free the datablock
  //   //kfree(dp->addrs[off]);  // remove the de from dp ??
  //   itrunc(ip);  // free the ip
  // }
  //
...
keep:
    ...
    if(ip->type == T_SYM) {
      ...
      iunlock(ip);// will the readi release the lock of ip?
        // cprintf("OPEN after readi: path = %s\n", path);  // problem w/ path, how to get inode given inum
        // cprintf("OPEN after readi: name = %s\n", name);  // problem w/ path, how to get inode given inum
      // update ip
      //char* fmname = fmtname(name);  // only looking at current directory?
      if((ip = namei(name)) == 0){
        end_op();
        return -1;
      }
...
sys_symlink(void)
{
  ...
  // cprintf("3 - name: %s\n", name);
  // ilock(l_dp);
  // if(l_dp->dev != ip->dev || dirlink(l_dp, link_name, ip->inum) < 0){
  //   //cprintf("3a: %d\n", l_dp->dev != ip->dev);
  //   //cprintf("3b: %d\n", dirlink(l_dp, name, ip->inum) < 0);
  //   iunlockput(l_dp);
  //   goto bad;
  // }
  // cprintf("4\n");
